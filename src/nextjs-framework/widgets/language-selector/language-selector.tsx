...
export async function LanguageSelector(props: WidgetContext<LanguageSelectorEntity>) {
    ...
    const languages = cultures.map((culture: string, index: number) => {
    ...
    return (
      <div {...dataAttributes} className="qu-dropdown-selector dropdown d-flex justify-content-center">
        <a className="nav-link dropdown-toggle" href="#" id="language_selector"
          data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
          <svg xmlns="https://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-globe"
            viewBox="0 0 16 16">
        ...
        <ul className="dropdown-menu" aria-labelledby="language_selector">
          {languages.map((item: LanguageEntry, idx: number) => {
                    return (<li key={idx} className="nav-item">
                      <a className={classNames('dropdown-item nav-link', {
                            'active': item.Selected
                        })} href={item.PageUrl} target="_self">
                        {item.Name}
                      </a>
                    </li>);
                })
